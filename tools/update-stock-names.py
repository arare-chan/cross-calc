#!/usr/bin/env python3
"""public/calculator.html の銘柄名辞書(STOCK_NAMES)を JPX 公式データから更新するスクリプト。
...
使い方:
  ...
  3. public/calculator.html 内の STOCK_NAMES ブロックが最新データで置き換わる。
...
HTML_PATH = os.path.join(os.path.dirname(__file__), "..", "public", "calculator.html")
TS_PATH = os.path.join(os.path.dirname(__file__), "..", "lib", "stock-names.ts")
...
def main() -> None:
    ...
    ts = (
        "// Generated by tools/update-stock-names.py. Do not edit by hand.\n"
        + "export const STOCK_NAMES: Readonly<Record<string, string>> = "
        + js
        + ";\n"
    )
    open(TS_PATH, "w", encoding="utf-8").write(ts)
    print(f"更新完了: {len(names)}銘柄(基準日 {base_date})→ {HTML_PATH}, {TS_PATH}")
